...
import {
  fontFamilyFor, normalizeDna, resolvedDarkPalette, typographyCharacters, visualAntiPatternOptions,
  type Dna, type PaletteRoles, type TypographyCharacter, type VisualDepth,
} from '../data/visualDna'
import {
...
export default function VisualStudio({ dna: rawDna, onChange, suggestionContext, recommendations, onOpenPattern, onAddPattern, sanity }: Props) {
  ...
  const setTypography = (character: TypographyCharacter) => patch({ typographyCharacter:character, headingTypography:character, bodyTypography:(character.includes('serif') || character==='Display / expressive')?'Humanist sans':character, typography:`${character}${character.includes('serif') || character==='Display / expressive'?' + utility sans':''}` })
  ...
  return <>
    <header className="page-header"><div><div className="eyebrow">Visual Studio · v0.17</div><h1>Make the interface look intentionally yours.</h1><p>Browse advice when you want it, preview without commitment, then apply only the part you explicitly choose. Manual controls remain the deterministic source of truth.</p></div></header>
...
function LivePreview({ dna }: { dna:Dna }) {
  ...
  return <div className={`dna-preview visual-preview motion-${dna.motionCharacter.toLowerCase()} amount-${dna.motionAmount.toLowerCase()}`} style={{background:stageBg,backgroundSize:dna.backgroundTreatment==='Texture'?'9px 9px':undefined,color:p.ink,fontFamily:body,['--preview-duration' as string]:`${dna.motionAmount==='None'?0:dna.motionDuration}ms`}}>
    ...
    <div className="visual-app-shell" style={{width:contentWidth,padding:`0 ${pad}px ${pad}px`,margin:'0 auto'}}>
      <nav className="visual-app-nav" style={{minHeight:Math.max(48,dna.inputHeight+8),borderColor:p.border}}><div className="preview-logo" style={{background:dna.accentUsage==='Rare'?p.ink:p.accent,borderRadius:dna.shapeLanguage==='Sharp'?0:dna.shapeLanguage==='Rounded'?12:6}}/><span>Northstar</span><div className="preview-nav-actions"><Search {...iconProps}/><button style={{borderRadius:controlRadius,borderColor:p.border,color:p.ink,fontWeight:dna.buttonWeight==='Bold'?800:dna.buttonWeight==='Quiet'?500:700}}>New item</button></div></nav><div key={`${dna.motionCharacter}-${dna.motionAmount}-${dna.motionDuration}`} className="preview-motion-sample" style={{color:p.muted}}><span>MOTION · {dna.motionCharacter}</span><b><i style={{background:p.accent}}/></b></div>
